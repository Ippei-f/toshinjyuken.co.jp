...
$userid[]   = 'toushin_hsh';// ログインID
$password[] = 'htd2023slt';// パスワード
...
//ページタイトル
$pagetitle='東新住建 発電シェルターハウス 物件情報';
...
$categoryArr = COMMON_PARAM('search_cate',0);

//エリア読み込み
include_once(dirname(__FILE__).'/../../../../temp_php/data-area.php');

//画像タイプ
$categoryArr_photo = COMMON_PARAM('search_photo_type');

//ブランド
$categoryArr_brand = COMMON_PARAM('search_brand',1);

//保存用配列
$arr=COMMON_PARAM('dbDataStructure');
$dbDataStructure=$comment_name_arr=array();
foreach($arr as $v){
	$v2=str_replace('@','',$v);//配列判定につけた「@」外し
	$dbDataStructure[]=$v2;
	if(strpos($v,'@')!==false){
		$comment_name_arr[]=$v2;
	}
...
$extensionList = array('jpg','gif','png','pdf');
