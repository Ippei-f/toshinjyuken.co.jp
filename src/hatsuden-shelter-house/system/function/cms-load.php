...
//共通変数読み込み
require $kaisou.'system/function/common-param.php';

$file_path = $img_updir = '';

function CMS_SETUP($type){
	global $file_path,$img_updir,$dir_sns;
	//システム読み込み・軽量版
	$file_path = $dir_sns.$type.'/data/data.dat';
	$img_updir = $dir_sns.$type.'/upload/';
...
',$r1);
	...
	return $sysdata_proto;
}

function CMS_OPEN($a=array()){
	//プレビュー時は全表示（PHP7.0対応ver）
	global $sysdata;
	if(count($a)<1){$a=$sysdata;}
	$flag=true;
...
function CMS_IMGSET($id='',$prm=array()){/**/
	...
	*/
	...
	$files = glob($img_updir.'{*}',GLOB_BRACE);
	//$files = glob($img_updir.'*');//サーバー環境的に↑が駄目な場合はこっち
	if($id==''){
