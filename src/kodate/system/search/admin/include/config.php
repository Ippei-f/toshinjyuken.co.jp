...
#############################################################################################################
##
#  PKOBO-News01（画像アップ・エディタ機能搭載版）ver1.0.0 (2015.08.25公開) PHP新着情報、お知らせプログラム
#
...
$userid[]   = 'skeleton';// ログインID
$password[] = 'new2020ske';// パスワード
...
//共通変数読み込み
require_once(dirname(__FILE__).'/../../function/common-param.php');

//ページタイトル
$pagetitle='東新住建の家 物件情報';
...
$weekArray = COMMON_PARAM('weekArray');//function/common-param.phpで設定
...
//$categoryArr = COMMON_PARAM('phases');
$categoryArr_area=COMMON_PARAM('area2025');
foreach($categoryArr_area['エリア'] as $k => $v){
	$categoryArr_area[1][$k]=$v[0];
}
foreach($categoryArr_area['ブランド'] as $k => $v){
	$categoryArr_area[2][$k]=$v[0];
}
$categoryArr=$categoryArr_area['フェーズ'];
/*
$categoryArr_area[3][0]='無し';
$categoryArr_area[3]+=$categoryArr_area['その他'];
*/

//アップファイルの種類
$upfile_arr=COMMON_PARAM('upfile');
$comment_name_arr=array('comment','kukaku_text','kukaku_vr','gallery_type','gallery_text','access_text','banner_text','banner_target','near_text','title_sub','kukaku_outlet'
,'brand','other_cate','comment_img');

//保存用配列
$dbDataStructure	= COMMON_PARAM('dbDataStructure');
...
$linklFileCount = 1;
$maxCommentCount = 99;//本文、画像セットのMAX値
