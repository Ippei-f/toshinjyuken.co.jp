...
function CHECKBOX_ONOFF($k,$prm=array()){
...
function ADDBTN($a,$add=''){
	$btn='';
	foreach($a as $k => $v){
		$btn.=MAKE_BTN($k,$v['btn']);
	}
	$add=($add!='')?'_'.$add:'';
	return '<div class="addbtn'.$add.'">'.$btn.'</div>';
}
function ADDBTN_JQ($a){
	...
	foreach($a as $k => $v){
		...
		$jq.='$(".addbtn_bef .'.$k.'").click(function(){
		$(this).parent().parent().before(\''.MAKE_INPUT($v).'\');
		cleditorExe();
	});'.chr(10);
	}
