...
<table class="borderTable03">
	   ...
	   <th style="width:13%">登録日</th>
		 <th style="width:6em;">エリア</th>
		 <th style="width:11em;"><div style="font-size: 0.85em;">ブランド・フェイズ等</div></th>
...
?>
	   ...
	   <td style="font-size: 90%;"><?php echo date2FormatDate($dateType,$linesArray[1]);?><?php echo ($weekDsp == 1) ? ' ('.$weekArray[date('w',strtotime($linesArray[1]))].')' : '';?></td>
		 ...
		 if(!empty($linesArray[18])&&(strpos($linesArray[18],'※')===false)){
		 	echo '<hr style="margin:4px 0 5px;">'.$linesArray[18];
		 }
		 ?></td>
		 <td style="font-size: 90%;"><?php
		 if(!empty($linesArray[18])){
		 	$linesArray[23]=trim($linesArray[23]);
			if(!empty($linesArray[23])){
				$check='|'.str_replace($dataSeparateStr,'|',$linesArray[23]).'|';
				foreach($categoryArr_area[2] as $k => $v){
					if(strpos($check,'|'.$k.'|')!==false){
						$add=$categoryArr_area['ブランド'][$k];
						$add=isset($add['col'])?' style="color:'.$add['col'].'"':'';
						echo '<div'.$add.'>'.$v.'</div>';
					}
				}
			}
		 	/*
		 	$add=$categoryArr_area['ブランド'][$linesArray[23]];
		 	$add=isset($add['col'])?' style="color:'.$add['col'].'"':'';
		 	echo '<div'.$add.'>'.TextToKanma($categoryArr_area[2][$linesArray[23]]).'</div>';
			*/
		 	echo '<div>'.TextToKanma($categoryArr[$linesArray[3]]).'</div>';
			$linesArray[24]=trim($linesArray[24]);
			if(!empty($linesArray[24])){
				$check='|'.str_replace($dataSeparateStr,'|',$linesArray[24]).'|';
		 ...
		 <td class="top_photo"><?php
			foreach($extensionList as $val){
				$upFilePath = '../upload/order'.$linesArray[0].'-0s.'.$val;
			...
			?></td>
	   <td><?php
		 if(!is_array($linesArray[19])){$linesArray[19]=array($linesArray[19]);}
		 echo '<div style="font-size:66%;">'.TextToKanma($linesArray[19][0]).'</div>';
		 echo TextToKanma($linesArray[2]);
		 ?><?php if(newmark($linesArray[1],$new_mark_days)) echo ' <span class="newMark">New!</span>';?></td>
