...
function getLines2DspData($file_path,$img_updir,$config,$id='',$category=''){
	...
	foreach($lines as $key => $val){
		...
		$dspTag[$key]['url_target'] = $linesArray[5];
...
//サムネイルの存在チェックと表示
function dspThumb($data,$width){
	global $dspThumbNail;
	if($dspThumbNail == 1){
		if(isset($data['file_type'][0]) && isset($data['upfile_path_thumb'][0]) && $data['file_type'][0] == 'img' && file_exists($data['upfile_path_thumb'][0])){
			echo '<img src="'.$data['upfile_path_thumb'][0].'?'.uniqid().'" width="'.$width.'" class="thumbNail" id="thumb_'.$data['id'].'" />';
		}else{
			echo '　';//スペース確保のため全角スペースを入れておく（NO IMAGEなどの画像でもいいかも）
		}
	}
}
