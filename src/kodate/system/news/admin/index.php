...
<p>並び順は登録日順です。<?php
//echo ($new_mark_dsp == 1) ? '（現在の設定では登録日から<span class="col19">'.$new_mark_days.'</span>日間はタイトルに「<span class="newMark">New!</span>」が表示されます）' : '';
echo  '（現在の設定ではNEWマーク表示にチェックを入れている記事のみ「<span class="newMark">New!</span>」が表示されます）';
?></p>
...
?>
	   ...
	   <td style="font-size: 90%;"><?php echo date2FormatDate($dateType,$linesArray[1]);?><?php echo ($weekDsp == 1) ? ' ('.$weekArray[date('w',strtotime($linesArray[1]))].')' : '';?></td>
		 ...
		 */
		 ?>
	   <td><?php echo TextToKanma($linesArray[2]);?><?php
		 //if(newmark($linesArray[1],$new_mark_days)) echo ' <span class="newMark">New!</span>';
		 if($linesArray[11]==1){echo ' <span class="newMark">New!</span>';}
		 ?></td>
