...
$parts=array
('banner'=>array
 						('form'=>'<span class="dpIB label sort">画像</span><input type="file" name="data[banner][]" /><div></div><span class="dpIB label">リンク先</span><input type="text" size="40" name="data[banner_text][]" />[select]'
						,'btn' =>'▲ 項目追加'
						,'select_num'	=>15
...
for($i=0;$i<$max;$i++){
	$a=$parts['banner'];
	$a['form']=str_replace('<input type="file" name="data[banner][]" />',IMG_THUMB('banner'),$a['form']);
