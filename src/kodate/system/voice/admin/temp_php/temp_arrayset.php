		...
		foreach($extensionList as $val){
			...
			if(file_exists($upFilePath['L'])){
				if($val == 'jpg' || $val == 'png' || $val == 'gif'){
					...
					$upfileTag[$k][$i] = "<a href=\"{$upFilePath['L']}\" target=\"_blank\">アップファイル</a>\n";
				}
				if($type_edit){
...
function ADDBTN($a,$pre=false){
	...
	return '<div class="addbtn '.($pre?'prepend':'append').'">'.$btn.'</div>';
}
function ADDBTN_JQ($a){
	$jq='';
	foreach($a as $k => $v){
		$jq.='$(".addbtn.append .'.$k.'").click(function(){
		$(this).parent().parent().children(\'.padbox\').append(\''.MAKE_INPUT($v).'\');
		cleditorExe();
	});'.chr(10);
		$jq.='$(".addbtn.prepend .'.$k.'").click(function(){
		$(this).parent().parent().children(\'.padbox\').prepend(\''.MAKE_INPUT($v).'\');
		cleditorExe();
	});'.chr(10);
	}
...
function MAKE_DATE_INPUT($k,$select=0,$start=0,$end=0){
	global $dbDataStructure;
	$d_key=$dbDataStructure[$k];	
	$res='<select name="data['.$d_key.'][]">';
	for($i=$start;$i<=$end;$i++){
		$add='';
		if($i==$select){$add=' selected="selected"';}
		$res.='<option value="'.$i.'"'.$add.'>'.$i.'</option>'.chr(10);
	}
	$res.='</select>'.chr(10);
	return $res;
}
