...
if (empty($get_arr['btn'])) {
	$get_arr['btn'] = '';
}
if (empty($get_arr['send'])) {
	$get_arr['send'] = '';
}
if (empty($get_arr['input'])) {
	$get_arr['input'] = '';
}
		...
		<form action="<?php echo $_SERVER["SCRIPT_NAME"]; ?>" method="post" class="<?php echo ($step == 3) ? 'conf' : ''; ?>">
			...
			?>
				...
				<div class="advance-info">
					<div class="advance-info__badge">
						<img src="images/member/badge-advance.svg" alt="先行情報">
					</div>
					<div class="advance-info__txt">
						<p>ただいま会員登録（無料）で！</p>
						<ul>
							<li>先行公開物件を閲覧可能</li>
							<li>ご希望エリアの新着物件の先行案内</li>
						</ul>
						<div class="note">ご登録後、IDとパスワードをメールにて<br>
							お送りいたします
						</div>
					</div>
				</div>
