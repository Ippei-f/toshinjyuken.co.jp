...
$loan_prm = array(
	...
	//--------------
	,
	'変動金利'	=> 1.69 //変動金利（地方銀行など）
	,
	'固定金利'	=> 3.4 //固定金利（フラット35）
	,
	'1割分金利' => 2.9
	//--------------
	,
...
<script>
	$(window).load(function() {
		...
		$('.loanbox .simu_btn').click(function() {
			...
			var a = new Array();
			/*str+=*/
			a[0] = Number($(this).parents('.loanbox').find('input.loan_input4').val()) * 10000; //借入額
			/*str+=*/
			a[1] = Number($(this).parents('.loanbox').find('input.loan_input5').val()) * 10000; //ボーナス加算額	
			/*str+=*/
			a[2] = Number($(this).parents('.loanbox').find('select.loan_input7').val()); //借入期間（年）
			a[0] -= a[1] * 2 * a[2]; //借入額からボーナス時の返済額を除外（＝ 借入額 -（ボーナス * 2 * 借入期間））

			/*str+=*/
			a[3] = Number($(this).parents('.loanbox').find('input.loan_input6:checked').val()); //金利（年）
			/*str+=*/
			a[4] = Number($(this).parents('.loanbox').find('input.loan_input6:checked').attr('month')); //月変動
	...
	function LOAN_INPUT(obj) {
		...
		/*str+=*/
		a[0] = obj.parents('.loanbox').find('input.loan_input1:checked').val();
		/*str+=*/
		a[1] = obj.parents('.loanbox').find('input.loan_input2').val();
		/*str+=*/
		a[2] = obj.parents('.loanbox').find('input.loan_input2B').val();
		/*str+=*/
		a[3] = obj.parents('.loanbox').find('input.loan_input3').val();
		...
		/*str+=*/
		obj.parents('.loanbox').find('input.loan_input5').val();
		/*str+=*/
		v = obj.parents('.loanbox').find('input.loan_input6:checked').val();
		obj.parents('.loanbox').find('span.loan_input6').text(v);
		/*str+=*/
		obj.parents('.loanbox').find('select.loan_input7').val();
