...
function HEADER_MENU_PC($arr=array()){
	...
	foreach($arr as $v){
		$a=$link_list[$v];
		switch(true){
			case isset($a['hmenu']):$t=$a['hmenu'];
			break;
			case isset($a['menu']):$t=$a['menu'];
			break;
			default:$t=$a[1];
		}
...
function HEADER_MENU_SP2($arr=array()){
	...
	foreach($arr as $v){
		$a=$link_list[$v];
		if(!empty($a['menu-en'])){$a['en']=$a['menu-en'];}
		else if(empty($a['en'])){$a['en']=$a[1];}
		if(!empty($a['menu'])){$a['jp']=$a['menu'];}
		else if(empty($a['jp'])){$a['jp']=$a[1];}
