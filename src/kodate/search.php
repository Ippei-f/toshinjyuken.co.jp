...
foreach($sysdata_proto as $key => $sysdata){
	...
	*/	
	if(isset($_REQUEST['ブランド'])){
		...
		if(!is_array($sysdata[23])){$sysdata[23]=array($sysdata[23]);}
		$flag=true;
...
<ul>'.PHP_EOL;
		foreach($v2 as $k3 => $v3){
			...
			switch($v3['フェーズ']){
				case 1:
				case 2:
				$phase2025['P']='<img src="images/content/search/2025/phase'.$v3['フェーズ'].'.svg" class="add">';
				break;
				case 3:
				case 4:
				$phase2025['T']='<div class="b2">'.$area_list_2025['フェーズ'][$v3['フェーズ']].'</div>';
...
<div class="t">
<div class="brand">'.$v3['ブランド'].$phase2025['T'].'</div>
<h3>'.$v3['タイトル'].'</h3>
