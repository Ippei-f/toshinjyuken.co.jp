...
foreach($sysdata_proto as $key => $sysdata){
	...
	*/
	...
	if(isset($_REQUEST['その他'])){
		...
		if(!is_array($sysdata[24])){$sysdata[24]=array(trim($sysdata[24]));}
		$flag=true;
	...
	if(isset($_REQUEST['価格'])){
		//価格帯チェック（タイプ名が空でない価格のみ対象）
		$price_match=false;
		$has_valid_price=false;
		if(is_array($sysdata[9])){
			$max_idx=count($sysdata[9]);
			for($i=0;$i<$max_idx;$i+=4){
				$type_name=$sysdata[9][$i];//タイプ名
				$p=$sysdata[9][$i+2];//価格
				if($type_name==''){continue;}//タイプ不明はパスする（表示ロジックと同様）
				if($p==''){continue;}//価格無しもパス（表示ロジックと同様）
				if(!is_numeric($p)){continue;}//数値比較のため数値以外はスキップ
				$has_valid_price=true;
				foreach($_REQUEST['価格'] as $range){
					switch($range){
						case '2,000万円台':
							if($p>=2000&&$p<3000){$price_match=true;}
							break;
						case '3,000万円台':
							if($p>=3000&&$p<4000){$price_match=true;}
							break;
						case '4,000万円台':
							if($p>=4000&&$p<5000){$price_match=true;}
							break;
						case '5,000万円以上':
							if($p>=5000){$price_match=true;}
							break;
					}
		...
		if(!$has_valid_price||!$price_match){continue;}
	}
	$a=array();
	$a['日付']=$sysdata[1];//日付
	...
	if(!isset($area_cnt[$area_db[0]][$v])){$area_cnt[$area_db[0]][$v]=0;}
	$area_cnt[$area_db[0]][$v]++;
	
	if(isset($_REQUEST['エリア'])){
		//エリア・サブエリア絞り込み
		if(strpos($_REQUEST['エリア'][0],$area_db[0])===false){continue;}//エリア外は除外
		if(strpos($_REQUEST['エリア'][0],'全選択')===false){
			if(!isset($a['サブエリア'])){continue;}//サブエリア無しは除外
			$check='｜'.implode('｜',$_REQUEST['エリア']).'｜';
			if(strpos($check,$a['サブエリア'])===false){continue;}//サブエリア外も除外
		}
	}
...
<ul>'.PHP_EOL;
		foreach($v2 as $k3 => $v3){
			...
			if(is_array($v3['タイプ'])){
				...
				$b[]=$v3['タイプ'][$i];
				$b[]=$v3['タイプ'][$i+2];
				if($b[0]==''){continue;}//タイプ不明はパスする
				if($b[1]==''){continue;}//価格無しもパス
				//if($b[0]==''){$b[0]='Atype';}
				//$v3['タイプ'][$n[1]]=str_replace(',','',$v3['タイプ'][$n[1]]);
				if(is_numeric($b[1])){$b[1]=number_format($b[1]);}
				if($b[1]!=''){$b[1].='万円';}
				$a[]=$b[0].'：'.$b[1];
			}
			}
			...
			if(strpos($check,'｜3｜')!==false){$phase2025['T'].='<div class="b2">'.$area_list_2025['フェーズ'][3].'</div>';}
			if(strpos($check,'｜4｜')!==false){$phase2025['T'].='<div class="b2">'.$area_list_2025['フェーズ'][4].'</div>';}
			//販売済み処理
			$sold_out=false;
			if(strpos($check,'｜5｜')!==false){
				$phase2025['P']='';
			...
			*/
			...
			if(!empty($v3['その他カテゴリ'])){
				if(!is_array($v3['その他カテゴリ'])){$v3['その他カテゴリ']=array($v3['その他カテゴリ']);}
			...
			$v3['ブランド'].=$phase2025['T'];
			if(!empty($v3['ブランド'])){$v3['ブランド']='<div class="brand">'.$v3['ブランド'].'</div>';}
			
			//販売済みはリンク削除
			if($sold_out){
				$photo_link=$v3['写真'].$phase2025['P'];
			}else{
				$photo_link='<a href="'.$link_list['物件詳細'][0].'?id='.$k3.'"'.$login_window.'>'.$v3['写真'].$phase2025['P'].'</a>';
			}
			echo '<li>
<div class="p'.$class['p'].'">'.$photo_link.'</div>
<div class="t">
'.$v3['ブランド'].'
<h3>'.$v3['タイトル'].'</h3>
...
<section class="menu"><form action="<?php echo $_SERVER['SCRIPT_NAME']; ?>" method="post">
...
if(!empty($search_res)){
	...
	foreach($search_res as $k => $v1){
		if(!is_array($v1)){$a[]=$v1;continue;}
		foreach($v1 as $v2){
			if(!is_array($v2)){$a[]=$v2;continue;}
			foreach($v2 as $v3){$a[]=$v3;}
		}
	}
	$a=implode('<br>',$a);
}
...
function LOCAL_SEARCH_MENU1_LIST($a,$t){
	...
	$req_t=(isset($_REQUEST[$t])&&is_array($_REQUEST[$t]))?$_REQUEST[$t]:array();
	$c='['.implode('][',$req_t).']';
	//print_r($c);
	foreach($a as $v){
		...
		if(isset($v[1])&&is_array($v[1])){
			foreach($v[1] as $vv){
				...
				if(!isset($area_cnt[$v[0]][$vv])||($area_cnt[$v[0]][$vv]<1)){continue;}//1件もない所は省略（2025/06/13追加
...
?>
<div class="sp_pop"><div class="dark"><div class="frame"><img src="images/content/search/2025/close.svg" class="pc_vanish"><div class="scr">
<?php
LOCAL_SEARCH_MENU1('エリア');
LOCAL_SEARCH_MENU2('ブランド');
LOCAL_SEARCH_MENU2('価格');
...
<label><input type="button" value=" "><div class="pc_br_del"><div>検索条件を<br>リセット</div></div><img src="images/content/search/2025/arrow-R.svg"></label>
</div>
</div></div></div></div>
</form></section>
</div>
</main>
<div class="sp_fixbtn"><div>検索条件を変更・追加<img src="images/common/arrow-bottom.svg"></div></div>
<script>
...
$(window).load(function(){
	...
	$('.search_list2025 .menu dd input').on('click', function(){
		...
		if(f){$(this).parents('dd').find('input:not([value*="全選択"])').prop('checked',!f);}
		else{$(this).parents('dd').find('input[value*="全選択"]').prop('checked',f);}
	});
...
function LOCAL_SP_FIXBTN_FLAG(){
	...
	if(window.matchMedia('(min-width:1000px)').matches){$('.search_list2025 .sp_pop').removeAttr('style');}
}
</script>
<!-- ** -->
