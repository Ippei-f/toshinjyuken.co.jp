			...
			<div class="W1200 Wmax100per mgnAuto">
				...
				?>
					...
					<div class="LS_detail">
						...
						if (!is_array($sysdata_dt[6])) {
							$sysdata_dt[6] = array($sysdata_dt[6]);
						}
						if (!is_array($sysdata_dt[8])) {
							$sysdata_dt[8] = array($sysdata_dt[8]);
						}
						if (!is_array($sysdata_dt[11])) {
							$sysdata_dt[11] = array($sysdata_dt[11]);
						}
						?>
						...
						foreach ($sysdata_dt[9] as $type) {
							switch ($type) {
								case 'T':
						?>
									<h4 class="LSD_subt sp_br_del">
										<div><?php echo $sysdata_dt[8][$t_cnt++]; ?></div>
									</h4>
								<?php
									break;
								case 'P':
									if (empty($sysdata_dt['naiyou-num'][$p_cnt])) {
										$sysdata_dt['naiyou-num'][$p_cnt] = '';
									} //PHP7.0対応
				...
				<ul class="LS_list">
					...
					foreach ($sysdata_proto as $key => $sysdata) {
						...
						if (($get_arr['id'] != '') && ($get_arr['id'] == $sysdata[0])) {
							continue;
						}
						//一部データ配列化
						CMS_DATA_REPLACE();
