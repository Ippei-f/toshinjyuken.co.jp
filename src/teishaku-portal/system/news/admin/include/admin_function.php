...
function authAdmin($userid,$password){
	...
	$demo_add=array('','');
	if($democheck!=''){
		$demo_add[0]='#login_form form,#login_form p:nth-child(1){display:none;}';
		$demo_add[1]=explode('demo/toushin-gakku/',$_SERVER['SCRIPT_FILENAME']);
		$demo_add[1]='https://www.toshinjyuken.co.jp/gakku-project/'.$demo_add[1][1];
		$demo_add[1]='<p class="taC" style="margin-bottom:2em;">こちらはデモページです。下記のリンクから本サーバーのシステムに移動できます。</p>
		<p class="taC"><a href="'.$demo_add[1].'" class="col8">本サーバーのシステムに移動</a></p>';
	}
...
<title>{$pagetitle} 管理画面ログイン認証</title>
...
<body id="auth"><div align="center" class="basecolor" style="font-size:24px; padding-top:1em; padding-bottom:0.5em;"><b>{$pagetitle} 管理画面ログイン認証</b></div>{$error}
<div id="login_form">
