...
//		$kaiinn_memo ='【'.$p_title.' 追加分】'.chr(10).'↓'.chr(10).chr(10);
		$kaiinn_memo .= "●ご希望エリア："
.(($_REQUEST['kibouarea-0']!='')?chr(10).'第一希望：'.$_REQUEST['kibouarea-0']:'')
.(($_REQUEST['kibouarea-1']!='')?chr(10).'第二希望：'.$_REQUEST['kibouarea-1']:'')
.(($_REQUEST['kibouarea-2']!='')?chr(10).'第三希望：'.$_REQUEST['kibouarea-2']:'')
.chr(10);
$_REQUEST['kibouarea_set'].chr(10);
...
//		$toiawase_memo ='【'.$p_title.' 追加分】'.chr(10).'↓'.chr(10).chr(10);
		...
		//●ご希望物件名：尾張東エリア 小牧市東田中[toi_bukken_set]
		if(($_REQUEST['toi_bukken_set'] != "")&&($_REQUEST['toi_bukken_set'] != " ")){
			$toiawase_memo .= "●ご希望物件名：".$_REQUEST['toi_bukken_set'].chr(10);
		}
		//●お問い合わせ内容：-[nai_cb_set] 
...
//    'customer[custom:Raijou]' => $_REQUEST['yoyaku_set'].(($_REQUEST['yoyaku_set']!='')?'（ ご来場希望日時：'.$_REQUEST['kengakubi_set'].'）':''),
    'customer[append_tags]' => 'teishaku'
);
