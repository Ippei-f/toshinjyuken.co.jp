...
$(window).load(function(){
	...
	$('.loanbox input.cb_radio').click(function(){
		if($(this).hasClass('loan_input1')){
			...
			val=$(this).val();
			$(this).parents('.loanbox').find('.LIbox2 tr td:nth-of-type(2) span').html(val);
			$(this).parents('.loanbox').find('input.loan_input2').val(val);
			val=$(this).parents('span').find('input[type="hidden"]').val();
			val=comma(val);
			$(this).parents('.loanbox').find('.res_price3').html(val);
		}
