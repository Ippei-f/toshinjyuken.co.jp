...
})(window,document,'script','dataLayer','GTM-WG4NLRH');</script>
...
<meta name="description" content="「そだつ」は、2人くらいにちょうどいい住まい。従来のファミリー層はもちろん、定年後のご夫婦、結婚前のカップル、シングルマザー、LGBT、友人同士、兄弟姉妹、ペットとの暮らしなど、これから育まれる「新・かぞく」にぴったりな住まいです。">'.chr(10);
/*
~20220819
<meta name="description" content="「そだつ」は、従来のファミリー層はもちろん、定年後のご夫婦、結婚前のカップル、シングルマザー、LGBT、友人同士、兄弟姉妹、ペットとの暮らしなど、これから育まれる「新・かぞく」にぴったりな住まいです。">'.chr(10);
*/
if($democheck!=''){
...
$temp_title='【公式】'.$comp_data['HP名'].'‐東新住建';
...
<noscript><iframe src="https://www.googletagmanager.com/ns.html?id=GTM-WG4NLRH"
...
<a name="pt" id="pt"></a><div id="pagetop_btn"><a href="#pt" class="bg_cover bright"></a></div>'.chr(10);
...
function HEADER_SUBMENU($arr=array()){
	...
	foreach($arr as $k => $v){
		if(empty($v[1])){$v[1]=$k;}
		$add=(strpos($v[1],'<span>')!==false)?' col2':'';
		//<div class="icon"><img src="'.$dir_c.$v[0].'.svg"></div>
		$str.='<td><a href="'.$link_list[$k][0].'"><div class="text'.$add.'">'.$v[1].'</div></a></td>'.chr(10);
	}
	return $str;
}
function HEADER_SPMENU($arr=array()){
	global $link_list;
	$str='';
	foreach($arr as $v){
		$a=$link_list[$v];
		$text=isset($a['jp'])?$a['jp']:$a[1];
		$str.='<a href="'.$a[0].'">'.$text.'</a>'.chr(10);
	}
	return $str;
}
$logo=file_get_contents($dir_c.'logo-h.svg');
$logo=str_replace($str='<svg',$str.' class="logo" title="'.$comp_data['HP名'].'"',$logo);
$logo='<a href="'.$link_list['TOP'][0].'">'.$logo.'</a>';
/*
switch($p_type){
	case 'news':
	$logo.='<a href="'.$link_list['TOP'][0].'"><img src="'.$dir_c.'logo-w.svg" class="logo w" title="'.$comp_data['HP名'].'"></a>';
	break;
}
*/
$temp_header='<div class="H_head"></div><header><div class="pos_rel H_head">
'.$logo.'
<div class="accbox"><div><div class="m">
'.HEADER_SPMENU(array
('TOP'
,'NEWS'
,'物件情報'
...
<div class="submenu"><table border="0" cellpadding="0" cellspacing="0" class="pc_tblbreak fontP080 LH125 sp_LH100"><tr>
'.HEADER_SUBMENU(array
('物件情報'			=>array('icon-search')
,'お問い合わせ'	=>array('icon-contact','来場予約<span>・</span><br>お問合せ')
,'会員登録'			=>array('icon-member')
)).'
...
//◆フッタ
/*
$temp_bottom_bnr2='<ul class="top_bnrbox_2021">
<li><a href="news.php?id=12"><img src="images/top/so/bnr-weekday_2.jpg"></a></li>
</ul>'.chr(10);
*/
$arr=array
('発電SH'
...
$temp_bottom_bnr='<div class="W1000 Wmax100per mgnAuto bottom_bnr">'.chr(10);
foreach($arr as $v){
	if(is_array($v)){
		$a=$v;
	}
		...
		$a=$link_list['東新住建-'.$v];
		$a['url']=$a[0];
		$a['bnr']=$kaisou.'images/common/bottom/bnr-'.$a['bnr'].'.jpg';
	}	
	$temp_bottom_bnr.='<a href="'.$a['url'].'"><img src="'.$a['bnr'].'"></a>'.chr(10);
}
$temp_bottom_bnr.='</div>'.chr(10);

$temp_footer='<footer><div class="Wbase W625">
<table border="0" cellpadding="0" cellspacing="0" class="sp_tblbreak logoset"><tr>
...
<table border="0" cellpadding="0" cellspacing="0" class="sp_tblbreak logoset"><tr>
<td><a href="'.$link_list['東新住建'][0].'"><img src="'.$dir_c.'logo-toushin-2024.svg"></a></td>
<td></td>
<td class="addr_box LH150">
'.WORD_BR('〒'.$comp_data['〒'].'
'./*str_replace('(','(',)*/$comp_data['住所'].'
TEL.'.$comp_data['TEL']).'
</td>
</tr></table>
<div class="kyoka LH175">'.WORD_BR('<span>'.$comp_data['建築許可番号'][0].'</span><span>'.$comp_data['建築許可番号'][1].'</span>
<span>'.$comp_data['建築許可番号'][2].'</span><span>'.$comp_data['建築許可番号'][3].'</span>').'</div>
...
switch($p_title){
		...
		break;
		default:
		require $kaisou."temp_php/bottom-slide.php";
		$temp_footer=$temp_bottom_slide.$temp_bottom_bnr.$temp_footer;
}
...
function EFFECT_BTN($k,$text='',$prm=array()){
	...
	$href=($k!='x')?' href="'.$link_list[$k][0].$getprm.'"':'';
	
	$res='';
	if(!isset($prm['ver'])){$prm['ver']='';}//未設定（デフォルト）
	switch($prm['ver']){
		case 2023:
		$res='<div class="btn_bgLtoR_2023'.$class.'"><a'.$href.$style.'><div class="bg"></div><div class="text"><span>'.$text.'</span></div>'.$arrow.'</a></div>';
		break;
		default:
		$res='<a'.$href.' class="btn_bgLtoR'.$class.'"'.$style.'><div class="border"></div><div><span>'.$text.'</span>'.$arrow.'</div></a>';
	}
	return $res;
}
...
function MAINPIC($dir,$prm=array()){
	...
	if(empty($prm['alt'])){$prm['alt']='';}
	$alt=($prm['alt']!='')?' alt="'.$prm['alt'].'"':'';
	
	if(!empty($prm['clear'])){//透明画像によるトリミング
		$img='url('.$mainpic.')';
		if(file_exists($url=($dir.$imgarr[0].'-t.png'))){$img='url('.$url.'),'.$img;}
		if(file_exists($url=($clearpic.'-'.$prm['clear'].'.png'))){$clearpic=$url;}
		else{$clearpic.='.png';}
