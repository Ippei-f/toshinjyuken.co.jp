...
//CSVデータ取得（優先順位はTOPのみの機能）
$csv_data=file_get_contents('system/search/data/yuusen.csv');
//$csv_data=mb_convert_encoding($csv_data,'UTF-8','auto');
$csv_data=mb_convert_encoding($csv_data,'UTF-8','SJIS');//東新住建のサーバーでは'SJISの部分にautoは使えない'
$csv_data=explode('
',$csv_data);
unset($csv_data[0]);
$csv_data_rev=array();
foreach($csv_data as $k => $v){
	...
	$v=explode(',',trim($v));
	$csv_data_rev[$v[1]]=$v[0];
}
unset($csv_data);//もう使わない
...
foreach(CMS_SETUP('search') as $key => $sysdata){
	...
	if(isset($csv_data_rev[$sysdata[0]])){
		$bukken_arr['優先'][$csv_data_rev[$sysdata[0]]]=$sysdata;//優先データ
	}
	else{
		$bukken_arr['通常'][]=$sysdata;//通常データ
	}
}
ksort($bukken_arr['優先']);//昇順
$bukken_arr=array_merge($bukken_arr['優先'],$bukken_arr['通常']);

$cnt=0;
foreach($bukken_arr as $key => $sysdata){
