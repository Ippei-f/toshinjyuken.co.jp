...
<tr>
	...
	foreach($commentArr[9] as $k => $v){
		switch($v){
			...
			echo '<div class="kiji_T">'.COMMENT_SET(8).'</div>'.chr(10);
			break;
			case 'W':
			echo '<div class="kiji_W">'.COMMENT_SET(8).'</div>'.chr(10);
			break;
			case 'P':
			$img=IMG_THUMB('naiyou');
			$sizetext='';
			if(strpos($img,'<img')!==false){
				$sizetext='原寸表示';
				if($upsizeArr1[$cnum[10]]=='o'){
					if($commentArr[11][$cnum[11]]!=''){
						$sizetext=$commentArr[11][$cnum[11]].'px';
					}
				}
				$sizetext='<div style="margin-top: 0; display: inline-block;">≪ 画像サイズ … '.$sizetext.'</div>';	
			}					
			echo '<div class="kiji_P">'.$img.$sizetext.'</div>'.chr(10);
			$cnum[10]++;
			$cnum[11]++;
			break;
		}
