...
//		$kaiinn_memo ='【'.$p_title.' 追加分】'.chr(10).'↓'.chr(10).chr(10);
		$kaiinn_memo .= "●ご入居予定人数：大人".$_REQUEST['nyuukyo1-0']."人　子供：".$_REQUEST['nyuukyo1-1']."人".chr(10);
		$kaiinn_memo .= "●ご希望エリア：第二希望 ".$_REQUEST['kibouarea-0'];
		if($_REQUEST['kibouarea-1'] != ""){
			$kaiinn_memo .= "　第二希望：".$_REQUEST['kibouarea-1'];
		}
		if($_REQUEST['kibouarea-2'] != ""){
			$kaiinn_memo .= "　第三希望：".$_REQUEST['kibouarea-2'];
		}
		$kaiinn_memo .= chr(10);
		//●ご希望学区（小・中学校）
		if($_REQUEST['gakku1'] != ""){
			$kaiinn_memo .= "●ご希望学区（小・中学校）：".$_REQUEST['gakku1'].chr(10);
		}
		//●購入予定時期
		if($_REQUEST['kounyuuyotei'] != ""){
			$kaiinn_memo .= "●購入予定時期：".$_REQUEST['kounyuuyotei'];
			//●その他
			if($_REQUEST['kounyuuyotei_o'] != ""){
				$kaiinn_memo .= "　".$_REQUEST['kounyuuyotei_o'];
			}
			$kaiinn_memo .= chr(10);
		}
		//●ご希望のお住まい
		if($_REQUEST['kibousumai'] != ""){
			$kaiinn_memo .= "●ご希望のお住まい：".$_REQUEST['kibousumai'].chr(10);
		}
		//●土地
		if($_REQUEST['tochi'] != ""){
			$kaiinn_memo .= "●土地：".$_REQUEST['tochi'].chr(10);
		}
		//●ご予算
		if($_REQUEST['budget'] != ""){
			$kaiinn_memo .= "●ご予算：".$_REQUEST['budget'].chr(10);
		}
...
//ご来場前アンケート ココカラ--------------------
		//●物件を知ったキッカケ
		if(!empty($_REQUEST['anke_jizen'])){
			$toiawase_memo .= "■アンケート".chr(10);
			$toiawase_memo .= "●物件を知ったキッカケ：";
			foreach ($_REQUEST['anke_jizen'] as $value){
				$toiawase_memo .= $value."　";
				if($value == "知人の紹介"){
					$toiawase_memo .= $_REQUEST['anke_jizen_t8']."　";
				}
				if($value == "その他"){
					$toiawase_memo .= $_REQUEST['anke_jizen_o']."　";
				}
			}
			$toiawase_memo .= chr(10);
		}
		//●検討開始時期
		if(!empty($_REQUEST[''])){
			$toiawase_memo .= "●検討開始時期：";
		...
		//●お気軽試算計画表をもらう
		if(!empty($_REQUEST['okigaru'])){
			$toiawase_memo .= "●お気軽試算計画表をもらう：";
			foreach ($_REQUEST['okigaru'] as $value){
				$toiawase_memo .= $value."　";
			}
			$toiawase_memo .= chr(10);
		}
		//●来訪時に聞きたいこと
		if(!empty($_REQUEST['anke_kikitai'])){
			$toiawase_memo .= "●来訪時に聞きたいこと：";
			foreach ($_REQUEST['anke_kikitai'] as $value){
				$toiawase_memo .= $value."　";
			}
			$toiawase_memo .= chr(10);
		}
//ご来場前アンケート ココマデ--------------------
		//●ご入居予定人数
		if($_REQUEST['nyuukyo1-0'] != ""){
			$toiawase_memo .= "●ご入居予定人数：大人".$_REQUEST['nyuukyo1-0']."人　子供".$_REQUEST['nyuukyo1-1']."人".chr(10);
		}
		//●ご職業
		if($_REQUEST['job'] != ""){
